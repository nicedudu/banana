...
class TplResolve extends Template {
  ...
  resolve() {
    ...
    this.writeTpl(this.template, 'indexhtml', path.join(this.pageDir, 'index', 'index.ejs'), {
      name: this.name,
      version: this.version,
      title: '🍌 Banana-cli',
      logo: `<%= require('../../public/logo.svg')%>`,
      header: '<%- include ../../shared/header %>',
      footer: '<%- include ../../shared/footer %>'
    });
    this.writeTpl(this.template, 'headerhtml', path.join(this.sharedDir, 'header.ejs'), {
      name: this.name
    });
    this.writeTpl(this.template, 'footerhtml', path.join(this.sharedDir, 'footer.ejs'));
    ...
    this.writeTpl(this.template, 'optionsjs', path.join(this.pageDir, 'index', `options.js`));
  ...
  commit() {
    console.log();
    console.log(`${chalk.green('✔ ')} ${chalk.grey(`创建文件: ${this.name}/src/pages/index/index.ejs`)}`);
    console.log(`${chalk.green('✔ ')} ${chalk.grey(`创建文件: ${this.name}/src/shared/header.ejs`)}`);
    console.log(`${chalk.green('✔ ')} ${chalk.grey(`创建文件: ${this.name}/src/shared/footer.ejs`)}`);
    ...
    console.log(`${chalk.green('✔ ')} ${chalk.grey(`创建文件: ${this.name}/src/pages/options.js`)}`);
